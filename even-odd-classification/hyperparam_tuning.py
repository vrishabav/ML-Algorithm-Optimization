...
class XGBoostTree:
    def __init__(self, max_depth=6, min_child_weight=1, gamma=0, reg_lambda=1.0, colsample=1.0, use_exact_greedy=False, sketch_eps=0.1):
        ...
        for iteration in range(self.n_estimators):
            ...
            if (iteration) % 5 == 4:
                if X_val is not None and y_val is not None:
                    ...
                    print(f" Val Accuracy after {iteration+1} Trees: {val_accuracy:.4f}")
...
param_grid = {
    ...
    'gamma': [0, 0.5],
    'reg_lambda': [1.0, 2.0],
    'use_exact_greedy': [False],  # hardcoded to false since Exact Greedy algorithm did not provide any real improvement, and came at a greater time cost
    'sketch_eps': [0.15, 0.3]
}
                ...
                for subsample in param_grid['subsample']:
                    for min_child in param_grid['min_child_weight']:
                        for g in param_grid['gamma']:
                            for l2 in param_grid['reg_lambda']:
                                for use_exact in param_grid['use_exact_greedy']:
                                    # Determine which sketch_eps values to loop over
                                    if use_exact:
                                        # sketch_eps is not used, just run with the first (default) value
                                        eps_values = [param_grid['sketch_eps'][0]] 
                                    else:
                                        # sketch_eps is used, loop over all specified values
                                        eps_values = param_grid['sketch_eps']
                                    ...
                                    model.fit(Xtrain, ytrain, X_val=Xval, y_val=yval) 
                                    train_time = time.time() - train_start

                                    # Evaluate model
                                    ypred = model.predict(Xval)
                                    accuracy = accuracy_score(yval, ypred)
                                    print(f"  Final Accuracy: {accuracy:.4f}, Train Time: {train_time:.2f}s")

                                    # Resulst
                                    run_result = params.copy()
                                    run_result['Accuracy'] = round(accuracy, 4)
                                    run_result['Training Time (s)'] = round(train_time, 2)
